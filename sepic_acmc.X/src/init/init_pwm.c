...
volatile uint16_t init_sepic_pwm(void) {
    ...
    PG1CONHbits.UPDMOD = 0b001;     // PWM Buffer Update Mode Selection: Immediate update
                                     /* The PWM logic is pipe lined which means that the
                                     * signals are generated and propagated through
                                     * many levels of logic. There may be more than half
                                     * a dozen clocks between an update request and its
    ...
    PG1IOCONHbits.PMOD = 0b01;      // PWM Generator Output Mode Selection: PWM Generator outputs operate in Independent mode
    ...
    PG1EVTLbits.ADTR1EN1    = 0b1;          // PG1TRIGA  Compare Event is enabled as trigger source for ADC Trigger 1 -> Iin sample trigger
    PG1EVTLbits.UPDTRG      = 0b11;         // A write of the PGxTRIGA register automatically sets the UPDATE bit
    ...
    PG1EVTHbits.ADTR2EN2    = 0b1;          // PG1TRIGB register compare event is enabled as trigger source for ADC Trigger 2 -> Vout sample trigger
...
//    PG1CLPCILbits.PSS       = 0b11011;      // Selecting Comparator 3 output as PCI input
    ...
    PG1LEBL     = 0;   // No Leading Edge Blanking is use in this control configuration
...
volatile uint16_t launch_sepic_pwm(void) {
    ...
    while(timeout++<5000); // Delay is introduced to ensure PG1DC is written properly
    timeout = 0;

    PG1DC       = MIN_DUTY_CYCLE;               // Initial value for soft-start routine
    PG1TRIGB    = (PWM_PERIOD + PG1DC) >> 1;    // Output voltage should be sampled at 50% OFF time
    PG1TRIGA    = PG1DC >> 1;                   // Input current should be sampled at 50% ON time 
                                                // Internal register updates occur when PG1TRIGA is written
   
    while((PG1STATbits.UPDATE)  && (timeout++<5000));
