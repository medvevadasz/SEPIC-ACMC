...
;------------------------------------------------------------------------------
	...
	.global _c2p2z_voltage_Update
_c2p2z_voltage_Update:    ; provide global scope to routine
...
; Check for upper limit violation
	...
	bra C2P2Z_VOLTAGE_CLAMP_MAX_OVERRIDE    ; jump to override label if control output > upper limit
	bclr w12, #NPMZ16_STATUS_USAT    ; clear upper limit saturation flag bit
	bra C2P2Z_VOLTAGE_CLAMP_MAX_EXIT    ; jump to exit
	C2P2Z_VOLTAGE_CLAMP_MAX_OVERRIDE:
	mov w6, w4    ; override controller output
	bset w12, #NPMZ16_STATUS_USAT    ; set upper limit saturation flag bit
	C2P2Z_VOLTAGE_CLAMP_MAX_EXIT:
...
; Check for lower limit violation
	...
	bra C2P2Z_VOLTAGE_CLAMP_MIN_OVERRIDE    ; jump to override label if control output < lower limit
	bclr w12, #NPMZ16_STATUS_LSAT    ; clear lower limit saturation flag bit
	bra C2P2Z_VOLTAGE_CLAMP_MIN_EXIT    ; jump to exit
	C2P2Z_VOLTAGE_CLAMP_MIN_OVERRIDE:
	mov w6, w4    ; override controller output
	bset w12, #NPMZ16_STATUS_LSAT    ; set lower limit saturation flag bit
	C2P2Z_VOLTAGE_CLAMP_MIN_EXIT:
...
;------------------------------------------------------------------------------
; Global function declaration _c2p2z_voltage_Reset
; This function clears control and error histories enforcing a reset
;------------------------------------------------------------------------------
	
	.global _c2p2z_voltage_Reset
_c2p2z_voltage_Reset:
...
;------------------------------------------------------------------------------
; Global function declaration _c2p2z_voltage_Precharge
; This function loads user-defined default values into control and error histories
;------------------------------------------------------------------------------
	
	.global _c2p2z_voltage_Precharge
_c2p2z_voltage_Precharge:
