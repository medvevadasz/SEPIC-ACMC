...
#ifndef __SPECIAL_FUNCTION_LAYER_C2P2Z_IOUT_H__
#define __SPECIAL_FUNCTION_LAYER_C2P2Z_IOUT_H__
 ...
 * ***************************************************************************************/
	...
	} __attribute__((packed))C2P2Z_IOUT_CONTROL_LOOP_COEFFICIENTS_t;

	typedef struct
	{
		volatile fractional ControlHistory[2];  // Control History
		volatile fractional ErrorHistory[3];    // Error History
	} __attribute__((packed))C2P2Z_IOUT_CONTROL_LOOP_HISTORIES_t;


	extern volatile cNPNZ16b_t c2p2z_iout; // user-controller data object

/* ***************************************************************************************/

// Function call prototypes for initialization routines and control loops

extern inline uint16_t c2p2z_iout_Init(void); // Loads default coefficients into 2P2Z controller and resets histories to zero

extern inline void c2p2z_iout_Reset( // Resets the 2P2Z controller histories
	volatile cNPNZ16b_t* controller // Pointer to nPnZ data structure
	);

extern inline void c2p2z_iout_Precharge( // Pre-charges histories of the 2P2Z with defined steady-state data
	volatile cNPNZ16b_t* controller, // Pointer to nPnZ data structure
...
extern inline void c2p2z_iout_Update( // Calls the 2P2Z controller
	volatile cNPNZ16b_t* controller // Pointer to nPnZ data structure
	);

#endif	// end of __SPECIAL_FUNCTION_LAYER_C2P2Z_IOUT_H__ header file section
