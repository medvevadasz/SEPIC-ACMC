...
void ExecuteVbatASoftStart(void) {
    
    volatile float fdummy = 0.0;
    volatile uint16_t int_dummy = 0;
    
    
    switch (application.soft_start.step) {
        ...
        case SOFT_START_STEP_INITIALIZE:
            ...
            application.soft_start.v_reference = 0; // Reset Soft-Start Voltage Reference
            application.soft_start.i_reference = 0; // Reset Soft-Start Current Reference
            cvmc_bata.ptrControlReference = &application.soft_start.v_reference; // Voltage loop is pointing to Soft-Start Reference

            // switch to soft-start phase POWER-ON DELAY
            application.soft_start.step = SOFT_START_STEP_POWER_ON_DELAY;
        ...
        case SOFT_START_STEP_POWER_ON_DELAY:
            ...
            if(pwr_on_couter++ > SOFT_START_POWER_ON_DELAY)
            {
                ...
                BATA_PWM_LS_WR = 0;
                BATA_PWM_LS_INIT_OUTPUT;
                hspwm_set_gpio_low_side(BATA_PWM_CHANNEL);

                pchrg_couter = 0;   // Reset pre-charge counter
                application.soft_start.step = SOFT_START_STEP_PRECHARGE;
        ...
        case SOFT_START_STEP_PRECHARGE:
            ...
            else
            {
                ...
                hspwm_reset_gpio_low_side(BATA_PWM_CHANNEL);
                application.soft_start.step = SOFT_START_STEP_LAUNCH_V_RAMP; 
            }
        ...
        case SOFT_START_STEP_LAUNCH_V_RAMP:
            ...
            if (application.data.v_bata < application.bata_v_loop.reference)
            { application.soft_start.v_reference = application.data.v_bata; }
            else 
            { application.soft_start.v_reference = application.bata_v_loop.reference; }
            ...
            fdummy = (float)(application.data.v_bata) / (float)(application.data.v_dclink << 1);
            ...
            application.soft_start.step = SOFT_START_STEP_V_RAMP_UP;
        ...
        case SOFT_START_STEP_V_RAMP_UP:
            ...
            // increment reference
            application.soft_start.v_reference += SOFT_START_V_REF_STEP_SIZE;

            // check if ramp is complete
            if (application.soft_start.v_reference >= application.bata_v_loop.reference) 
            {
                application.soft_start.v_reference = application.bata_v_loop.reference;
                ...
                #if (BATA_CONTROL_MODE == VMC)
                application.soft_start.step = SOFT_START_WAIT_FOR_PWRGOOD;
                #elif (BATA_CONTROL_MODE == ACMC)
                application.soft_start.step = SOFT_START_STEP_I_RAMP_UP;
                #endif
            }
            break;
        ...
        case SOFT_START_WAIT_FOR_PWRGOOD:
            ...
            {
                application.soft_start.step = SOFT_START_STEP_COMPLETE; 
                pg_couter = 0; 
            }
            Nop();
            break;
        ...
        default:
            application.soft_start.step = SOFT_START_STEP_INITIALIZE;
            break;
    }
